...
public class ShowStudentTableService {
    ...
    public List<Map<String,Object>> getStudentTable(String professor_id, int limit, int page, String course_id,
        ...
        if(class_id!=null){
            query_1.eq("takes.class_id",class_id);
        }
        if(credits!=null){
        ...
        if(professor_id!=null&&((title!=null&&!title.equals(""))||(year!=null)||(semester!=null&&!semester.equals(""))||
            ...
            List<String> column = Arrays.asList("course.course_id","title","takes.class_id as class_id","semester","year","credits","student.student_id",
                    "student_name","grade");
            List<Map<String,Object>> course_table=teachesDao.listMap(column,query_1);
            return course_table;
        }
        else {
            List<String> column = Arrays.asList("distinct course.course_id", "title", "takes.class_id as class_id", "semester", "year", "credits");
            List<Map<String, Object>> course_table = teachesDao.listMap(column, query_1);
            return course_table;
        }
    }
